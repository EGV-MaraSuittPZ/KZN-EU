...
// ── Plazas Únete ─────────────────────────────────────
function renderUneteSlots() {
  var c = document.getElementById("unete-slots-container");
  if (!c || typeof uneteSlots === "undefined") return;
  c.innerHTML = uneteSlots.map(function(s) {
    var bc = s.type === "sponsor" ? "unete-slot-badge sponsor" : "unete-slot-badge";
    return '<div class="unete-slot">' +
      '<div class="unete-slot-icon">' + s.icon + '</div>' +
      '<div class="unete-slot-info">' +
        '<div class="unete-slot-title">' + s.title + '</div>' +
        '<div class="unete-slot-desc">' + s.desc + '</div>' +
      '</div>' +
      '<span class="' + bc + '">' + s.badge + '</span>' +
    '</div>';
  }).join("");
}
...
function initRecruit() {
  ...
  var success = document.getElementById("recruit-success");
  if (!form) return;
  form.addEventListener("submit", function(e) {
    e.preventDefault();
    var nick = document.getElementById("rf-nick").value;
    var rol  = document.getElementById("rf-rol").value;
    var pos  = document.getElementById("rf-posicion").value;
    var rank = document.getElementById("rf-rank").value;
    var disp = document.getElementById("rf-disponibilidad").value;
    var cont = document.getElementById("rf-contacto").value;
    var xtra = document.getElementById("rf-extra").value;
    var subj = encodeURIComponent("[KAIZEN] Candidatura — " + rol.toUpperCase() + " · " + nick);
    var body = encodeURIComponent(
      "NUEVA CANDIDATURA KAIZEN\n" +
      "========================\n\n" +
      "Nick:            " + nick + "\n" +
      "Rol:             " + rol  + "\n" +
      "Posicion:        " + (pos  || "—") + "\n" +
    ...
    window.location.href = "mailto:kaizen.esports.eu@gmail.com?subject=" + subj + "&body=" + body;
    setTimeout(function() {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (copy) copy.textContent = `© ${new Date().getFullYear()} KAIZEN Esports · Todos los derechos reservados`;
